//! Qwen2 (dense) architecture: standard GQA with biased qkv projections,
//! no per-head RMSNorm (unlike Qwen3). Covers Qwen2 / Qwen2.5 / Qwen2.5-Coder.
...
impl Attention {
    fn load(w: &mut WeightMap, prefix: &str, cfg: &Qwen2Config) -> Result<Self> {
...
impl Qwen2Model {
    pub fn load(mut weights: WeightMap, config_json: &Value) -> Result<Self> {
        let cfg = Qwen2Config::from_json(config_json)?;
