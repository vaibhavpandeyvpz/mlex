...
- Its only real "distribution" mechanism is the compiled `.node` binary, which ships inside the [`mlex.js` npm package](../../packages/node) — not as Rust source.
...
- **Node.js:** the compiled `.node` addon from this crate, plus the hand-written TypeScript types/wrapper in [`packages/node`](../../packages/node) → npm, as the [`mlex.js`](https://www.npmjs.com/package/mlex.js) package.
...
See the [`mlex.js` npm package README](../../packages/node/README.md) for the actual public JS/TS API, and the [top-level project README](../../README.md) for the full picture.
